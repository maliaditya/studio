...
import { ArrowRight, BrainCircuit, ChevronsLeft, ChevronsRight, ExternalLink, Minus, MousePointer2, Pin, PinOff, Plus, Redo2, Search, Undo2, X } from "lucide-react";
...
const readPersistedState = (): PersistedState | null => {
  ...
  try {
    ...
    if (!parsed || !Array.isArray(parsed.diagrams) || typeof parsed.activeDiagramId !== "string") return null;
    const diagrams = parsed.diagrams.filter(isDiagramDoc);
    if (diagrams.length === 0) return null;
    const activeExists = diagrams.some((d) => d.id === parsed.activeDiagramId);
    return {
      diagrams,
      activeDiagramId: activeExists ? parsed.activeDiagramId : diagrams[0].id,
    };
  } catch {
    return null;
  }
...
const sanitizeStore = (value: any): PersistedState | null => {
  ...
  return {
    diagrams,
    activeDiagramId,
    openDiagramIds: openDiagramIds.length > 0 ? openDiagramIds : [activeDiagramId],
    pinnedDiagramIds,
  };
};
...
export function StateDiagramModal({ isOpen, onOpenChange }: StateDiagramModalProps) {
  const { settings, setSettings, brainHacks, setOpenBrainHackPopups, mindsetCards, schedule } = useAuth();
  ...
  const [linkEditor, setLinkEditor] = useState<{ nodeId: NodeId; hackId: string; diagramId: string; botheringIds: string[] } | null>(null);
  ...
  useEffect(() => {
    ...
    setOpenDiagramIds((prev) => {
      ...
      return next.length > 0 ? next : diagrams[0] ? [diagrams[0].id] : [];
    });
    setPinnedDiagramIds((prev) => prev.filter((id) => valid.has(id)));
  ...
  const completeConnect = (toId: NodeId) => {
    ...
    setEdges((prev) => [...prev, { id, from: connectDraft.fromId, to: toId, label: "transition", color: "#94a3b8" }]);
  ...
  const onPointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (connectDraft) {
      const world = toWorld(e.clientX, e.clientY);
      setConnectDraft((prev) => (prev ? { ...prev, x: world.x, y: world.y } : prev));
      return;
    }
    ...
    if (drag) {
      ...
      const snappedX = e.altKey ? nextX : snapToGrid(nextX);
      const snappedY = e.altKey ? nextY : snapToGrid(nextY);
      drag.moved = true;
      ...
      setNodes((prev) => prev.map((n) => (group.includes(n.id) ? { ...n, x: n.x + dx, y: n.y + dy } : n)), { recordHistory: false });
      return;
    }
  ...
  const onWheelCapture = useCallback((event: React.WheelEvent) => {
  ...
  const addEdge = () => {
    ...
    const id = createId("edge");
    setEdges((prev) => [...prev, { id, from: newEdgeFrom, to: newEdgeTo, label: newEdgeLabel.trim() || "transition", color: newEdgeColor }]);
    setSelectedEdgeId(id);
    setSelectedNodeId(null);
  };
  ...
  const openLinkEditorForNode = (node: StateNode) => {
    setLinkEditor({ nodeId: node.id, hackId: node.linkedBrainHackId || "", diagramId: node.linkedDiagramId || "", botheringIds: node.linkedBotheringIds || [] });
  ...
  const saveLinkEditor = () => {
    ...
    setNodes((prev) => prev.map((n) => (n.id === linkEditor.nodeId ? { ...n, linkedDiagramId: linkEditor.diagramId || undefined, linkedBotheringIds: linkEditor.botheringIds || [] } : n)));
    setLinkEditor(null);
  };
      ...
      >
        ...
        <div className={`flex-1 min-h-0 grid grid-cols-1 ${gridColsClass}`}>
          <div className="min-w-0 flex flex-col">
            <div ref={frameRef} className="relative flex-1" style={{ touchAction: "none" }} onWheelCapture={onWheelCapture}>
              <div
                ...
                >
                  ...
                  <g transform={`translate(${camera.x}, ${camera.y}) scale(${camera.zoom})`}>
                    ...
                    {nodes.map((node) => {
                      ...
                      const hasNodeLinks = !!node.linkedBrainHackId || (node.linkedBotheringIds?.length || 0) > 0;
                      ...
                      return (
                        <g key={node.id} className="cursor-default">
                          ...
                          {resolveLinkedHackId(node.linkedBrainHackId) ? (
                            <g
                              onPointerDown={(e) => e.stopPropagation()}
                              onClick={(e) => {
                                e.stopPropagation();
                                ...
                                const worldY = node.y;
                                openLinkedBrainHack(node.linkedBrainHackId, {
                              ...
                              style={{ cursor: "pointer" }}
                            >
                              <circle cx={node.x + nodeWidth + 12} cy={node.y + 14} r={9} fill="#2563eb" stroke="#e2e8f0" strokeWidth={1.5} />
                              <foreignObject x={node.x + nodeWidth + 6} y={node.y + 8} width={12} height={12} onPointerDown={(e) => e.stopPropagation()}>
                                <div className="h-full w-full flex items-center justify-center text-slate-100">
                                  <ExternalLink size={9} />
                                </div>
                              </foreignObject>
