...
const CustomChartTooltip = ({ active, payload, label, pageType }: any) => {
    if (active && payload && payload.length) {
        const data = payload[0].payload;
        const config = pageType === 'upskill' ? upskillChartConfig : workoutChartConfig;
        ...
        return (
            <div className="grid min-w-[12rem] items-start gap-1.5 rounded-lg border bg-background px-2.5 py-1.5 text-xs shadow-xl">
                ...
                {pageType === 'workout' && data.sets && data.sets.length > 0 && (
                 ...
                 {pageType === 'upskill' && data.sets && data.sets.length > 0 && (
                     <div className="mt-2 pt-2 border-t">
                        <p className="font-medium text-foreground mb-1">Sessions Logged:</p>
                        <ul className="space-y-1">
                            {data.sets.slice().sort((a: LoggedSet, b: LoggedSet) => a.timestamp - b.timestamp).map((set: LoggedSet, index: number) => (
                                <li key={index} className="flex justify-between text-muted-foreground">
                                   <span>Session {index + 1}:</span> 
                                   <span className="font-mono font-medium">{set.weight} min</span>
                                </li>
                            ))}
                        </ul>
                    </div>
...
export function ExerciseProgressModal({
  ...
  const chartConfig = pageType === 'upskill' ? upskillChartConfig : workoutChartConfig;
  const yAxisLabel = pageType === 'upskill' ? "Max Duration" : "Max Weight";
  const yAxisLabel2 = pageType === 'upskill' ? "Total Duration" : "Total Volume";
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead>{pageType === 'workout' ? 'Sets Details (Reps x Weight)' : 'Session Details (Duration)'}</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {tableData.map((entry) => (
                            ...
                            <TableCell>
                              {entry.sets.map((set, idx) => (
                                <span key={set.id} className="mr-2 inline-block bg-muted/50 px-1.5 py-0.5 rounded text-xs">
                                  {pageType === 'workout' ? `${set.reps}r x ${set.weight}kg/lb` : `${set.weight} min`}
                                </span>
                              ))}
                            </TableCell>
                          </TableRow>
              ...
              <>
                {graphData.length < 2 ? (
                     <p className="text-center text-muted-foreground py-8">
                        Need at least two data points (sessions on different days) to draw a graph.
                    </p>
                ): (
                  <ChartContainer config={chartConfig} className="min-h-[400px] w-full">
                    <LineChart accessibilityLayer data={graphData} margin={{ top: 5, right: 40, left: 10, bottom: 5, }}>
