...
import { Brain, ArrowRight, Play, BookText, BrainCircuit, CheckSquare, Zap, Expand, Shrink, PlusCircle, Trash2, MoreVertical } from 'lucide-react';
...
const InteractiveTutorial = ({ cards }: InteractiveTutorialProps) => {
    ...
    const ANIMATION_DURATION_MS = (8 * 6 * 1000) + (16 * 4 * 1000) + (10 * 5 * 1000);
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mindsetCards.map((card) => (
                <Card key={card.id} className="flex flex-col rounded-2xl group overflow-hidden transition-all duration-300 hover:shadow-xl">
                    <CardHeader>
                        <div className="flex justify-between items-start gap-2">
                           <div className="flex items-center gap-2 flex-grow min-w-0">
                                {editingCardId === card.id ? (
                                    <Input value={card.title} onChange={(e) => handleUpdateCard(card.id, e.target.value)} onBlur={() => setEditingCardId(null)} autoFocus className="text-lg font-semibold h-9" />
                                ) : (
                           ...
                           <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    <DropdownMenuItem onSelect={() => setEditingCardId(card.id)}>Edit Title</DropdownMenuItem>
                                    <DropdownMenuItem onSelect={() => handleDeleteCard(card.id)} className="text-destructive">Delete Card</DropdownMenuItem>
                                </DropdownMenuContent>
                           </DropdownMenu>
                        </div>
                    </CardHeader>
                        ...
                        <ul className="space-y-3">
                            {card.points.map((point) => (
                                <li key={point.id} className="flex items-start gap-3 text-sm text-muted-foreground group/item">
                                    ...
                                    <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive opacity-0 group-hover/item:opacity-100" onClick={() => handleDeletePoint(card.id, point.id)}>
                    ...
                    <CardFooter>
                         <Button variant="outline" size="sm" className="w-full" onClick={() => handleAddPoint(card.id)}>
             ...
             >
                ...
                <p className="mt-4 text-md font-semibold text-muted-foreground group-hover:text-primary transition-colors">Add New Card</p>
            </Card>
        </div>
    );
};
