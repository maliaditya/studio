...
const BLOB_PATHNAME = 'lifeos-versions.json';
...
export async function GET() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Failed to download version plan. Status: ${response.status}`);
        }
        ...
        // Assuming the file format is { versions: Version[] }
        return NextResponse.json({ versions: data.versions || [] });

    } catch (error) {
        console.error("GET /api/publish-releases error:", error);
        return NextResponse.json({ error: "Failed to fetch version plan" }, { status: 500 });
    }
}

// POST handler to update the version plan
export async function POST(request: Request) {
    ...
    try {
        const { username, versions } = await request.json();
        ...
        if (!Array.isArray(versions)) {
            return NextResponse.json({ error: 'Invalid payload: "versions" must be an array.' }, { status: 400 });
        }

        const dataToStore = JSON.stringify({ versions }, null, 2);
        ...
        return NextResponse.json({ success: true, message: 'Life OS version plan updated successfully.' });

    } catch (error) {
        console.error("POST /api/publish-releases error:", error);
        const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred.';
        return NextResponse.json({ error: `Failed to update version plan: ${errorMessage}` }, { status: 500 });
    }
}
